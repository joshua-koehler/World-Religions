...
from religions_django_app.models import CombinedDataPoint, ProtestantPoint, PapistPoint
...
class Command(BaseCommand):
    ...
    def _create_combined_data_points(self):
        protestant_percent_values, papist_percent_values, years = add_protestant_and_papist_percentages_through_2050()
        for i in range(len(protestant_percent_values)):
            new_cdp = CombinedDataPoint(year=years[i][0],
                                        protestant_percent=float(protestant_percent_values[i][0]),
                                        papist_percent=float(papist_percent_values[i][0]))
            new_cdp.save()
...
def extract_data():
    religion = "/home/joshua/PycharmProjects/religions/religions_django_project/religions_django_app/management/National_World_Religions_1945-2010.csv"
    mexico_data = []
...
def add_protestant_and_papist_percentages_through_2050():
...
def add_all_combined():
    protestant_percent_values, papist_percent_values, years = add_protestant_and_papist_percentages_through_2050()
    # management = []

    for i in range(len(protestant_percent_values)):
        add_combined_data_point(years[i][0], float(protestant_percent_values[i][0]), float(papist_percent_values[i][0]))
        ...
        management.append(datum)

    for datum in management:
        # POST datum


    # convert to json
    json_data = json.dumps(management)
    return JsonResponse(json_data, safe=False)
    '''


def add_combined_data_point(year, protestant_percent, papist_percent):
    new_combined_data_point = CombinedDataPoint()
    new_combined_data_point.year = year
    new_combined_data_point.protestant_percent = protestant_percent
    new_combined_data_point.papist_percent = papist_percent
    new_combined_data_point.save()
